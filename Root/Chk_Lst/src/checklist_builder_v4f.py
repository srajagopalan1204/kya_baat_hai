...
def build_default_meta(spec_path: Path, excel_meta: dict):
    ...
    # Default titles now refer to v4f
    app_title = excel_meta.get("APP_TITLE", "SOP Build Checklist v4f")
    app_title_visible = excel_meta.get("APP_TITLE_VISIBLE", app_title)

    # Repo is the local working folder; can be overridden per Excel spec
    ...
    # Default image folder can be overridden per SOP family
    meta_img_folder = excel_meta.get("META_IMG_FOLDER_DEF", "SOP/images/SE/Distro/Quo2Ord")

    # NEW: web root / base URL for GitHub Pages (e.g. /EdxBuild)
